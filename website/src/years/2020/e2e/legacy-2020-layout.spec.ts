...
import { legacy2020Pages } from '../data/pages';

const legacy2020EditionRoutes = legacy2020Pages
  .map((page) => page.route)
  .filter(
    (route) => route.startsWith('/2020-spring/') || route.startsWith('/2020-fall/')
  );
...
test('legacy 2020 Spring post icons use bundled asset routes', async ({ page }) => {
  ...
  expect(iconHrefs).not.toEqual(
    expect.arrayContaining([expect.stringContaining('/src/years/2020/assets/live')])
  );
...
test('legacy 2020 uploads are served from event-owned routes', async ({ request }) => {
