...
const policies = [
  ...
  {
    route: 'code-of-conduct/attendee-reporting',
    heading: 'Procedures for Reporting Incidents',
  },
...
test.describe('2026 English policy content in localized site shells', () => {
  for (const policy of policies) {
    test(`keeps the same English ${policy.route} body across all locales`, async ({
      page,
    }) => {
      let englishBody = '';
      for (const locale of locales) {
        ...
        await expect(
          page.getByRole('heading', { level: 1, name: policy.heading, exact: true })
        ).toBeVisible();
        ...
        await expect(
          page.locator('[data-site-header] a[aria-label="PYCON HK 2026"]')
        ).toHaveAttribute('href', `/2026/${locale.code}/`);
        for (const target of locales) {
          expect(
            (
              await page
                .locator(`[data-locale-switch="${target.code}"]`)
                .first()
                .getAttribute('href')
            )?.replace(/\/$/, '')
          ).toBe(`/2026/${target.code}/${policy.route}`);
        }
  ...
  test('language switching preserves policy page and English body', async ({
    page,
  }) => {
