...
for (const locale of locales) {
  test(`programme publication state is explicit in ${locale}`, async ({ page }) => {
    ...
    await expect(page.locator('[data-programme-status]')).toHaveAttribute(
      'data-programme-status',
      sample ? 'published' : 'unpublished'
    );
    if (sample) {
...
test('sample supports real dates, search, filters, bookmarks, keyboard modal and calendar', async ({
  page,
}) => {
  ...
  await expect(page.locator('[data-programme-empty]')).toHaveText(
    'No sessions have been published for this day.'
  );
  ...
  await expect(page.locator('#modal-details [data-session-description]')).toContainText(
    'Python isn’t just a language'
  );
  await expect(page.locator('#modal-details [data-speaker-profile]')).toHaveCount(1);
  await expect(page.locator('#modal-details [data-speaker-biography]')).toContainText(
  ...
  await page.screenshot({
    path: join(tmpdir(), 'pyconhk-programme-desktop.png'),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: join(tmpdir(), 'pyconhk-programme-mobile.png'),
    fullPage: false,
  });
  expect(errors).toEqual([]);
});

test('session details replace speaker profiles when changing talks and show every co-speaker', async ({
  page,
}) => {
  test.skip(!sample, 'Requires the public 2025 sample build.');
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  expect(
    await page.locator('[data-modal-body]').evaluate((element) => element.scrollTop)
  ).toBe(0);
});
...
for (const locale of locales) {
  test(`speaker stays on site and links back to the session in ${locale}`, async ({
    page,
    context,
  }) => {
    test.skip(!sample, 'Requires the public 2025 sample build.');
    ...
    for (const suffix of ['', '/']) {
      const legacy = await context.request.get(
        `/2026/${locale}/speakers/a520d5ae8a6503a04d37${suffix}`,
        { maxRedirects: 0 }
      );
      expect(legacy.status()).toBe(301);
      expect(new URL(legacy.headers().location, legacy.url()).pathname).toBe(
        `/2026/${locale}/speakers/peter-ho/`
      );
    }
    for (const suffix of ['', '/index.html']) {
      const alternative = await context.request.get(
        `/2026/${locale}/speakers/peter-ho${suffix}?from=schedule`,
        { maxRedirects: 0 }
      );
      ...
      expect(
        (await context.request.get(canonical.href, { maxRedirects: 0 })).status()
      ).toBe(200);
    }
    await page.setViewportSize({ width: 390, height: 844 });
    ...
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
      'href',
      new RegExp(`/2026/${locale}/speakers/peter-ho/$`)
    );
    ...
    await expect(page.locator('[data-locale-switch="ja"]').first()).toHaveAttribute(
      'href',
      new RegExp(profilePath.replace(`/${locale}/`, '/ja/'))
    );
    for (const width of [320, 640, 768, 1024, 1536]) {
      ...
      expect(
        await page
          .locator('[data-speaker-biography]')
          .evaluate((element) => element.scrollWidth <= element.clientWidth + 1)
      ).toBe(true);
      if (locale === 'en' && [320, 1536].includes(width))
        await page.screenshot({
          path: join(tmpdir(), `pyconhk-speaker-page-${width}.png`),
          fullPage: true,
        });
    }
    await expect(page.locator('[data-speaker-session]').first()).toHaveCSS(
      'cursor',
      'pointer'
    );
    ...
    if (locale === 'en') {
      ...
      await page.screenshot({
        path: join(tmpdir(), 'pyconhk-branded-session.png'),
        fullPage: false,
      });
    }
    await page.keyboard.press('Escape');
