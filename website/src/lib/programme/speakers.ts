...
type Speaker = Profile & { id: string; slug: string; sessions: ScheduleItem[] };

export function speakerIdentity(person: Pick<Profile, 'name' | 'url'>, event: string) {
  return createHash('sha256')
    .update(`${event}:${person.url || person.name}`)
    .digest('hex')
    .slice(0, 20);
}

export function canonicalSpeakerName(name: string) {
  return (
    name
      .normalize('NFKD')
      .replace(/\p{M}/gu, '')
...
export function createSpeakerDirectory(
  ...
  for (const session of sessions) {
    for (const profile of sessionProfiles(session)) {
      ...
      if (!speaker.sessions.some((item) => item.id === session.id))
...
export function speakerRedirects(speakers: Speaker[], locales: string[]) {
  return locales.flatMap((locale) =>
    speakers.flatMap(({ id, slug }) => {
