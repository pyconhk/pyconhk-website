...
for (const width of [320, 768, 1440]) {
  test(`sponsorship packages remain reachable at ${width}px`, async ({ page }) => {
    ...
    for (let index = 0; index < count - visibleCount; index++) {
      ...
      await expect(page.locator('[data-sponsorship-position]')).toContainText(`${index + visibleCount + 1} / ${count}`);
    }
    await expect(next).toBeDisabled();
    await expect(page.locator('[data-sponsorship-position]')).toContainText(`${count} / ${count}`);
...
test('homepage story heading stays legible on its blue background', async ({ page }) => {
