...
test('registration is pending and calendar uses the actual 2026 event dates', async ({ page }) => {
...
test('mobile navigation opens a real access guide route', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Access Guide' })).toBeVisible();
});

test('CFP is closed and uses the current conference theme in every locale', async ({ page }) => {
  for (const locale of locales) {
    ...
    await expect(page.locator('main a[href="https://cfp.pycon.hk/pyconhk2026/me/submissions/"]').first()).toBeVisible();
  }
});
