...
const widths = [320, 360, 390, 639, 640, 767, 768, 1023, 1024, 1279, 1280, 1459, 1460, 1535, 1536, 1920];

test.describe('responsive public programme', () => {
  test.skip(!sample, 'Requires the public 2025 sample build.');

  for (const locale of locales) {
    test(`${locale}: all rooms and controls fit across breakpoint boundaries`, async ({ page }) => {
      const errors: string[] = [];
      ...
      for (const width of widths) {
        ...
        const roomColumnsFit = await programme.evaluate((element) => element.clientWidth >= 1412);
        await expect(programme).toHaveAttribute('data-room-layout', String(roomColumnsFit));
        ...
        const geometry = await page.evaluate(() => {
          ...
          const visible = [...document.querySelectorAll<HTMLElement>('[data-session-card], .programme-filters button, .programme-filters input, .programme-filters select')].filter((element) => element.getClientRects().length);
          return {
            documentFits: document.documentElement.scrollWidth <= innerWidth,
            programmeFits: programme.scrollWidth <= programme.clientWidth + 1,
            allElementsFit: visible.every((element) => {
              const box = element.getBoundingClientRect();
              return box.x >= 0 && box.right <= innerWidth && element.scrollWidth <= element.clientWidth + 1;
            }),
      ...
      expect(await page.locator('[data-session-card]:visible').count()).toBeGreaterThan(0);
      await page.locator('[data-clear-filters]').click();
      await expect(page.locator('.programme-scroll')).toHaveAttribute('data-room-layout', 'true');
    ...
    for (const viewport of [{ width: 320, height: 568 }, { width: 844, height: 390 }]) {
      test(`${locale}: long session details remain usable at ${viewport.width}x${viewport.height}`, async ({ page }) => {
