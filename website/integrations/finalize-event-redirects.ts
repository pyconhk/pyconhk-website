...
import { validateSnapshot } from '../src/lib/programme/snapshot.ts';
import { createSpeakerDirectory, speakerRedirects } from '../src/lib/programme/speakers.ts';

export async function finalizeEventRedirects(projectRoot: string) {
  ...
  async function main() {
    ...
    for await (const file of fs.glob(['2026/*/speakers/*.html', '2026/*/talks/*.html'], { cwd: distRoot })) {
      ...
      await fs.rename(source, path.join(directory, 'index.html'));
    }
