...
const defaultOutDir = fileURLToPath(
  new URL('../../output/playwright/2016-parity/', import.meta.url)
);
...
child.on('close', (code, signal) => {
  if (signal) {
    console.error(`2016 pixel diff stopped by ${signal}`);
    process.exit(1);
  }
...
child.on('error', (error) => {
  console.error(`Could not start 2016 pixel diff: ${error.message}`);
  process.exit(1);
});
