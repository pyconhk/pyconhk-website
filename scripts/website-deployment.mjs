...
import { validateSnapshot } from "../website/scripts/programme-snapshot.mjs";
...
async function prepare(environment, force) {
  ...
  const args = ["website/scripts/sync-schedule.mjs", "--event", target.event,
    "--url", target.source, "--environment", environment, "--output", snapshotPath,
    "--allow-unpublished"];
  ...
  const result = execFileSync(process.execPath, args, { cwd: root, encoding: "utf8", env: { ...process.env, GITHUB_OUTPUT: "" } });
  const sync = JSON.parse(result.trim());
  const manifest = {
    sourceSha,
    programmeHash: sync.programme_hash,
    event: target.event,
    environment,
...
async function main() {
  ...
  else throw new Error("Usage: website-deployment.mjs prepare <target> [--force] | finalize <snapshot> <manifest> | verify <target> <manifest>");
}
