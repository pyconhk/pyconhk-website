...
import { validateEditorialBoundary } from "../scripts/check-cms-pr.ts";
...
test("editorial content cannot carry a replacement checker, workflow or app code", () => {
  for (const file of ["scripts/check-cms-pr.ts", ".github/workflows/branch-rules.yml", "cms/src/lib/oauth.ts"]) {
